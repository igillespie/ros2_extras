...
from geometry_msgs.msg import TransformStamped, PoseStamped, PointStamped, Twist, Quaternion
...
class MoveToPoint(Node):
    def __init__(self):
        ...
        # Subscribe to clients telling us where to go
        ...
        self.path = []  # Example final target, can be updated
    ...
    def nav_callback(self, msg):
        ...
        do_convex = target_y < 0.0
        self.path = self.elliptical_arc(self.current_x, self.current_y, target_x, target_y, num_steps, 0.15, do_convex).tolist()
        ...
        # Publish the path for visualization in RViz and what not
    ...
    def move_to(self, point_in_odom_x: float, point_in_odom_y: float):
        ...
        if self.timer is None:
            self.timer = self.create_timer(0.05, self.check_position)
    ...
    def publish_path(self, waypoints):
        ...
        path_msg.header.frame_id = "odom"  # Change if using a different frame
        
        for i in range(len(waypoints)):
            pose = PoseStamped()
            pose.header.stamp = path_msg.header.stamp
            pose.header.frame_id = "odom"  # Same as Path's frame_id
            pose.pose.position.x = waypoints[i][0]
            pose.pose.position.y = waypoints[i][1]
            pose.pose.position.z = 0.0

            # Compute orientation using yaw (rotation around z-axis)
            if i < len(waypoints) - 1:
                dx = waypoints[i + 1][0] - waypoints[i][0]
                dy = waypoints[i + 1][1] - waypoints[i][1]
                yaw = math.atan2(dy, dx)  # Compute angle
        ...
        self.get_logger().info("Published path to RViz")
